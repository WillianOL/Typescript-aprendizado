...
function retorna<variavel>(n: variavel): variavel {
  return n;
}
...
console.log(retorna<number>(300));
...
function primeirosCincos<T>(lista: T[]) {
...
/*
  extends
*/
// Dessa forma, o generic(tipo) vai aceitar qualquer elemento que for HTMLElement, se for diferente não aceita
// E assim podemos definir que a função vai aceitar um dado dês de quê ele seja um tipo especifico
function mostrarDado<T extends HTMLElement>(elemento: T) {
  return {
...
if (button) console.log(mostrarDado(button));
...
const button2 = $<HTMLButtonElement>('button');
console.log(button2);
